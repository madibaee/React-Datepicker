...
export default (input, persianDate, format) => {
  ...
  function replaceFunction(token) {
    switch (token) {
      ...
      // September 4th 1986
      case 'LL': {
        return `(?:(${persianDate.rangeName().months.join('|')}) ${digit(
          2,
        )} ${digit(4)})`
      }
      // Sep 4 1986
      case 'll': {
        return `(?:(${persianDate.rangeName().monthsShort.join('|')}) ${digit(
          2,
        )} ${digit(4)})`
      }
      ...
      case 'LLL': {
        return `(?:(${persianDate.rangeName().months.join('|')}) ${digit(
          4,
        )} ${digit(2)}   ${digit(1, 2)}:${digit(1, 2)}  (ق ظ|ب ظ|AM|PM))`
      }
      // Sep 4 1986 8:30 PM
      case 'lll': {
        return `(?:(${persianDate.rangeName().monthsShort.join('|')}) ${digit(
      ...
      case 'LLLL': {
        return `(?:(${persianDate.rangeName().weekdays.join('|')}) ${digit(
          1,
          2,
        )} (${persianDate.rangeName().months.join('|')}) ${digit(4)}  ${digit(
          1,
          2,
        )}:${digit(1, 2)}  (ق ظ|ب ظ|AM|PM))`
      }
      // Thu, Sep 4 1986 8:30 PM
      case 'llll': {
        return `(?:(${persianDate.rangeName().weekdaysMin.join('|')}) ${digit(
          1,
          2,
        )} ((${persianDate.rangeName().monthsShort.join('|')})) ${digit(
          4,
        )}  ${digit(1, 2)}:${digit(1, 2)}  (ق ظ|ب ظ|AM|PM))`
      }
    }
  }
  ...
  function parse(value, token) {
    switch (token) {
      ...
      case 'H':
        return {hour: parseInt(value)}
      case 'HH':
        return {hour: parseInt(value)}
      case 'h':
        return {hourA: parseInt(value)}
      case 'hh':
        return {hourA: parseInt(value)}
      case 'm':
        return {minute: parseInt(value)}
      case 'mm':
        return {minute: parseInt(value)}
      case 's':
        return {second: parseInt(value)}
      case 'ss':
        return {second: parseInt(value)}
      case 'D':
      ...
      case 'DD':
        return {date: parseInt(value)}
      case 'DDD':
        return {dayYear: parseInt(value)}
      case 'd':
        return {day: parseInt(value)}
      case 'ddd':
        return {day: persianDate.rangeName().weekdaysMin.indexOf(value) + 1}
      case 'dddd':
        return {day: persianDate.rangeName().weekdays.indexOf(value) + 1}
      case 'ddddd':
        return {
          date: persianDate.rangeName().persianDaysName.indexOf(value) + 1,
        }
      case 'w':
        return {weak: parseInt(value)}
      case 'ww':
        return {weak: parseInt(value)}
      case 'M':
      ...
      case 'MM':
        return {month: parseInt(value)}
      case 'MMM':
        return {month: persianDate.rangeName().monthsShort.indexOf(value) + 1}
      case 'MMMM':
        return {month: persianDate.rangeName().months.indexOf(value) + 1}
      case 'YY':
